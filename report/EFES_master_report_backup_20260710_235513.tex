...
The tone output is a Sinusoidal PWM (\texttt{pwm\_generic\_master.vhd}). A free running 8 bit
counter (\texttt{pwm\_cnt\_s}) counts from 0 to 255 and then wraps back to 0, so it draws a
sawtooth ramp (not a triangle). A comparator turns this into the pin: \texttt{PWM\_o} is high
while \texttt{pwm\_cnt\_s} $<$ \texttt{duty\_env\_s}, so the width of each pulse is set by the
...
To keep the tone clean, I did not use a simple square wave, because a square wave
...
To solve this, I implemented a PWM approach that moves the switching
 ...
 carrier, and the tone is carried by the \emph{duty}.
  ...
  clean sine at $f$. The switching noise of the PWM shows up around the carrier, 158.2\,kHz,
  and its multiples, all far above the audio band. This is the reason PWM is better than a
  plain square wave: it pushes the extra energy up near the carrier, well away from the tone,
  instead of leaving it on the low harmonics inside the audio band. The speaker does not
  reproduce that high frequency noise, so the audio band holds only the wanted tone.



When the module receives the address \texttt{0x04} from the bus, it loads two 16-bit
 ...
 a hardware divider is costly on an FPGA. So, using a trick from online DSP tutorials
...
\end{tikzpicture}}
\caption{Sinusoidal PWM (\texttt{pwm\_generic\_master.vhd}).}
\label{fig:spwm}
\end{figure}
...
SPI. The FPGA block \texttt{spi\_display.vhd} is a transmit only SPI in mode 3 (SCK idles
...
is $40.5\,\mathrm{MHz}/6 = 6.75$\,MHz. In clock cycles, one SCK is 6 \texttt{clk\_i} and one
byte is $8\times6 = 48$ \texttt{clk\_i} $= 1.19\,\mu$s.
...
\end{tikzpicture}
\caption{NOR flash read \texttt{0x03} (\texttt{spi\_master\_generic.vhd}).}
\label{fig:nor-read}
\end{figure}
...
\end{tikzpicture}
\caption{NOR flash page program \texttt{0x02} (\texttt{spi\_master\_generic.vhd}).}
\label{fig:nor-write}
\end{figure}
...
blob that starts with a fixed byte \texttt{0xA5} (a marker the firmware checks to know the
...
\textbf{Unlock before reading the id.} On this board the \texttt{/HOLD} and \texttt{/WP}
pins are left unconnected. A floating \texttt{/HOLD} pin picks up noise from the SCK line,
which makes the chip stop in the middle of a transfer, and then every read gives
\texttt{0xFF}, even the chip id. So I cannot just read the id first to check that the chip is
there. Instead, at boot the firmware first sends a WRSR command that sets the QE bit (this
frees the \texttt{/HOLD} and \texttt{/WP} pins) and clears the write protection, then a
...
for \texttt{rv32imc}. The code sits in the 32\,KB ITCM and the data in the 16\,KB DTCM, both
...
the other, always in this order (Figure~\ref{fig:firmware-loop}). First \texttt{emit\_capture}
takes the characters that arrived from the ESP32 and puts them in a queue. Then
\texttt{emit\_player\_tick} takes one character from the queue and plays its tone pair on the
...
in the middle of a tone it stops the PWM at once. Next \texttt{decode\_step} checks
\texttt{fill\_cnt}, and when a new spectrum is ready it copies the snapshot, decodes it, and
sends the decoded characters to the ESP32. After that \texttt{nor\_poll} runs the flash
storage protocol (Section~\ref{sec:flash}), and finally \texttt{display\_tick} sends one small
chunk to the display (Section~\ref{sec:display}).

A \texttt{health\_tick} runs every 15\,s to check that the slaves are alive. It reads one safe
...
(\texttt{\$HLT}) that both the display and the web dashboard show, so I can see at a glance
...
Diagnostics use the character UART. They are simple lines that start with \texttt{\$} and end
...
character UART & \texttt{\$...} line & diagnostics (FPGA to ESP32) \\
NOR command UART & \texttt{L} & append a log record \\
NOR command UART & \texttt{S} & save the settings \\
NOR command UART & \texttt{C} & clear the log \\
NOR command UART & \texttt{G} & read log slots \\
NOR command UART & \texttt{H} & read the ring head \\
NOR command UART & \texttt{Q} & read the settings \\
NOR command UART & \texttt{I} & read the chip id \\
NOR command UART & \texttt{T} & read the status \\
\hline
\end{tabular}
\end{table}
...
\emph{warm up} symbol: the EOF tone is held for 300\,ms instead of 144\,ms, which gives the
