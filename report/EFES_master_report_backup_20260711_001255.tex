...
% let long URLs break across lines
...
\author{Gioele Giunta s360501, Politecnico di Torino, LM-32\\ \small Course: Electronics for Embedded Systems}
...
logical channel (the carrier) and one carrying the data symbol. The receiver
moves the microphone signal to the frequency domain with an FFT and reads the two
active tones as spectral peaks.

This version is a full custom FPGA implementation of the master side, paired
...
connected through a Cloudflare WebSocket. The only link between master
...
Inside the master, three units split the work by timing class. The FPGA
...
buck (12\,V\,$\to$\,5\,V into the board's VIN); the board itself only
regulates and exposes 3.3\,V for the digital peripherals, while the 5\,V that supplies
the analog (audio) front end is not a board generated rail but the buck's own output, the same node that feeds VIN. The 3.3\,V does not reach the peripherals directly, it passes through
an IRLZ44N MOSFET whose gate is held high by FPGA pin 80, so that output must stay at
'1' or the NOR flash, the display and the LEDs all lose power.
...
The interconnect (wb\_interconnect.vhd) is a shared Wishbone bus with two
masters and ten slaves. The masters are the PicoRV32 (M0) and the audio accelerator
(M1). The slave is selected by decoding addr[31:27]: 5 bits,
 the PicoRV32 IP treats every address with addr[31:29]=000 as internal
...
strobe is not: a decoder reads addr[31:27] and a set of multiplexers send the
handshake (cyc and stb) to the one matching slave only. That slave sees
its stb and cyc go to 1 while all the others stay at 0, so the per slave
strobe works like a chip select: only the addressed slave reacts and drives its read data
back onto the shared dat\_o. An address that matches no slave falls on a sink that
never raises ack.

{\footnotesize
\setlength{\LTleft}{\fill}\setlength{\LTright}{\fill}
\begin{longtable}{l l l l p{6.7cm}}
\caption{Address map. The slave is selected by the top 5 bits addr[31:27]}
\label{tab:addrmap}\\
\toprule
...
\bottomrule
\endlastfoot
0x2000\_0000 & S6 & character UART
 & 0x00 & tx byte (write) / pop rx FIFO (read, bit\,8 $=$ valid) \\
 & & & 0x04 & enable \\
 & & & 0x08 & disable \\
 & & & 0x0C & baud divider \\
 & & & 0x10 & format \\
 & & & 0x14 & status \\
\midrule
0x2800\_0000 & S7 & NOR command UART
 & \multicolumn{2}{l}{same register map as S6 (second instance, pins 72/20)} \\
\midrule
0x3000\_0000 & S5 & audio accelerator
 & 0x04 & start \\
 & & & 0x08 & stop \\
 & & & 0x0C & SDRAM base \\
\midrule
0x3800\_0000 & S8 & NOR flash SPI
 & 0x00 & rx byte \\
 & & & 0x04 & tx byte (starts a transfer, clears done) \\
 & & & 0x08 & CS bit \\
 & & & 0x0C & busy/done (non destructive) \\
\midrule
0x4000\_0000 & S1 & ADC SPI
 & 0x00 & sample (acks only when ready) \\
 & & & 0x04 & start \\
 & & & 0x08 & stop \\
 & & & 0x0C & clear ready \\
\midrule
0x4800\_0000 & S0 & SDRAM snapshot$^{\ast}$
 & 0\,0\,000100000\,00 & fetch FSM status \\
 & & & 0\,0\,000100001\,00 & samples counter \\
 & & & 0\,0\,000100010\,00 & triggers counter \\
 & & & 0\,0\,000100011\,00 & IRQ counter \\
 & & & 0\,0\,000100100\,00 & fill\_cnt (frame counter) \\
 & & & 1\,0\,000000000\,00 & spectrum bin 0 \\
 & & & $\vdots$ & $\vdots$ \\
 & & & 1\,0\,111111111\,00 & spectrum bin 511 \\
\midrule
0x5000\_0000 & S2 & speaker PWM
 & 0x04 & two 16 bit tones packed ($f_1$ in [15:0], $f_2$ in [31:16]);
 loads them and starts the DDS driving the PWM pin \\
 & & & 0x08 & stop (PWM pin idles) \\
\midrule
0x5800\_0000 & S9 & display SPI
 & 0x00 & byte to shift out to the panel (MSB first, on SDA) \\
 & & & 0x04 & drive the panel's DC / RST / CS lines \\
 & & & 0x08 & busy (shifter still sending) \\
\midrule
0x6000\_0000 & S3 & LED PWM & \multicolumn{2}{l}{same register map as S2} \\
\midrule
0x7000\_0000 & S4 & GPIO & 0x00 & drive pin (write) / read back (read) \\
\end{longtable}
}

\textbf{Arbitration} is static priority: when M1 asserts
cyc\,\&\,stb the mux gives it the bus, and M0 receives no ack and
...
dat\_i & master & data master $\to$ slave (on a write) \\
dat\_o & slave & data slave $\to$ master (on a read) \\
...
\item the master drives adr, sets we\,=\,1 and puts the byte on dat\_i;
\item it raises cyc\,\&\,stb;
\item the slave whose addr[31:27] matches latches dat\_i into the addressed register and raises ack;
\item the master sees ack and drops cyc\,\&\,stb.
\end{enumerate}

A \textbf{read} is identical but
we\,=\,0: the slave drives the data onto dat\_o and raises
ack, and the master samples dat\_o on that cycle.

A master does not know how long a slave will take, so it holds cyc\,\&\,stb
and waits until ack comes back from the slave.
...
a second output clkoutp is the same 40.5\,MHz shifted by
$360^{\circ}/16=22.5^{\circ}$ ($1.54$\,ns), and clocks only the SDRAM die to
...
% data_ready: from the TOP of the ADC SPI, up a little, then left to the accelerator top;
% it passes BEHIND the bus (a gap is left where it crosses the bus)
\draw[line width=0.7pt]
  (R2.north) -- ($(R2.north)+(0,0.45)$) -- ($(R2.north)+(-4.3,0.45)$) -- (2.7,7.9) -- (0.7,7.9);
\draw[-{Stealth[length=5pt]}, line width=0.7pt] (-0.7,7.9) -- (-5.15,7.9);
\node[font=\scriptsize, fill=white, inner sep=1.5pt, anchor=south] at (-7.5,8.0) {\texttt{data\_ready} (direct wire to \texttt{dma\_proc})};
...
buck makes 5\,V, and the 5\,V feeds the board VIN and the analog front end. An on
...
The 3.3\,V to the peripherals passes through one more IRLZ44N, gated by gpio\_1
(pin 80). This is a soft power switch: the CPU can cut power to the NOR flash, the display
and the LEDs by driving the pin low, so the pin must stay at '1' in normal use.
...
the memory operates using burst transfers. A single read (rd\_n)
 or write (wr\_n) command, accompanied by a 21 bit address specifying the bank,
 row, and column, triggers a data burst of data\_len$+1$ words.
 In my design, data\_len is set to 26, resulting in a 27 word burst.
The operation is managed by a few key control signals: busy\_n 
indicates when the controller is ready to accept a new command request;
 rd\_valid confirms that the current read word is valid data; 
 and init\_done signals that the power up initialization sequence is complete.
...
computes next\_state only, and a synchronous process registers the state,
one shared counter tmr (cleared on every state change), and all
...
problem on that side), sequential addressing and full page burst. This is why the
...
S\_IDLE. Table~\ref{tab:sdram-cmd} lists the commands it issues.
...
\begin{table}[H]
\centering
\caption{SDRAM timings from the datasheet, in 40.5\,MHz cycles.}
\label{tab:jedec}
\small
\begin{tabular}{llcc}
\toprule
...
\caption{Complete sdram\_controller\_fsm state graph.}
\label{fig:sdram-fsm}
\end{figure}

\subsection{Bring up sequence}

At power up the die must be initialised in a fixed order before it works. The FSM runs it once and then raises init\_done:
\begin{enumerate}\setlength{\itemsep}{0pt}
...
\end{enumerate}
This runs only at boot. After init\_done the controller issues normal commands
plus one AUTO REFRESH every 312 cycles. In the diagram the address line
addr[10] (O\_sdram\_addr[10]) doubles as a control bit: on a PRECHARGE
the chip reads addr[10]$=1$ as ``all four banks'', which is why it is high
...
(the DQ bus). Both waits, and the others further down, are the JEDEC numbers
...
second flip flop the FSM raises rd\_valid, which tells that
the word on O\_sdrc\_data is good. So rd\_valid goes high CL$+2$
...
bit came back wrong (0xFFFE instead of 0xFFFF). Feeding the die the $1.54$\,ns shifted
clock (clkoutp) lets me sample in the middle of the data, and the errors
stopped.
...
short chain of flip flops (wpipe1 $\to$ wpipe2 $\to$
wpipe3 $\to$ dq\_out\_r) that shifts every cycle
...
\textbf{The alignment stage WR\_ALIGN.} On the board word 0 arrived one cycle
early and was lost, shifting the whole burst by one bin. One extra flip flop
(WR\_ALIGN) puts it back in step; the simulation has no board delays and uses
WR\_ALIGN$=0$.
...
number, and each bin goes to position burst$\times26+$index. Only 26 of the 256
...
between them, inside the block I call the audio accelerator (dma.vhd,
Figure~\ref{fig:audio-zoom}). The capture FSM (dma\_proc, bus master M1)
waits for the SPI master's data\_ready\_s flag (a direct wire, not the bus), reads
the sample over the bus, clears that flag, and writes the sample into spi\_raw\_buf,
a $1024\times16$ block RAM holding two 512 sample windows. The FFT FSM
(fft\_proc) takes the window just filled, feeds the Gowin FFT core, collects
the 512 spectrum words into fft\_result\_buf, drains them to SDRAM through
...
\node[sip] (sip) at (17.0,2.6) {SDRAM ctrl FSM\\\scriptsize dedicated write port};
...
\node[lbl, align=center, font=\scriptsize, fill=white, inner sep=1pt] at (10.5,-1.95) {feed \texttt{xn\_re}\\collect \texttt{xk\_re}};
...
\draw[ctl] (cfg.east) -- ++(0.7,0) |- (fft.west);
\node[lbl, anchor=west, fill=white, inner sep=1pt] at (6.15,-2.45) {start / base};

\node[pin] (sdpin) at (19.0,2.6){}; \draw[dpb] (sip.east) -- (sdpin);
...
\caption{FFT and drain FSM (fft\_proc).}
\label{fig:fft-fsm}
\end{figure}
...
dedicated master (spi\_master.vhd); its internal block diagram
...
The FSM has three states: S\_SHIFT covers bit periods 0 to 13 with CS low (the 12
data bits are captured on periods 2 to 13), S\_GAP covers periods 14 to 15 with
CS high, and while the start register stays set the machine loops
S\_GAP$\to$S\_SHIFT with no dead cycle. When the frame ends
data\_ready\_s goes to 1. The FSM is in
...
\end{tikzpicture}
\caption{ADC SPI master FSM (spi\_master.vhd).}
...
\end{tikzpicture}
\caption{One MCP3201 frame, cycle by cycle.}
\label{fig:spi-wave}
\end{figure}
...
The read side of the SDRAM port is the fetch FSM
...
\[
\text{bin} = b\times26 + idx, \qquad
b=0\ldots19\ \text{(20 bursts)}, \quad
idx=0\ldots25\ \text{(26 words)},
\]
The 20 bursts give $20\times26=520$ words, 8 more than
the 512 bin BSRAM, so a bin$<512$ guard keeps bins 0 to 511 and drops the last 8
words of burst 19. When burst 19 finishes, fill\_cnt increments. Each burst runs
...
fill\_cnt is how the CPU knows a new spectrum is ready: it is a counter that
goes up by one each time a fetch finishes, and wraps around.
The CPU keeps the last value it read and
compares: when fill\_cnt has changed, a fresh 512 word snapshot is ready and
...
protocol bins: a local maximum test against the six neighbours within $\pm3$
bins, a
curvature test on $k{-}1,k,k{+}1$ (the peak must open downward, so $k$ is a
crest and not the skirt of a neighbour); and an amplitude test on the
...
The tone output is a Sinusoidal PWM (pwm\_generic\_master.vhd). A free running 8 bit
counter (pwm\_cnt\_s) counts from 0 to 255 and then wraps back to 0, so it draws a
sawtooth ramp (not a triangle). A comparator turns this into the pin: PWM\_o is high
while pwm\_cnt\_s $<$ duty\_env\_s, so the width of each pulse is set by the
8 bit duty (0 to 255). The counter wraps every 256 clocks, so the switching rate, the
carrier, is $f_\mathrm{clk}/2^{8} = 40.5\,\mathrm{MHz}/256 = 158.2$\,kHz for the speaker, far
above the audio band.

To keep the tone clean, I did not use a square wave, because a square wave
also carries strong odd harmonics at $3f$, $5f$, $7f$ and so on.
...
To solve this, I implemented a PWM approach that moves the switching
 ...
 carrier, and the tone is carried by the duty.
  The duty traces a sine from the DDS and the SINE\_LUT 
  (Figure~\ref{fig:spwm}), and the speaker cannot follow 158.2\,kHz, so it plays back a
  clean sine at $f$. The switching noise of the PWM sits around the carrier, 158.2\,kHz, and
  its multiples, all far above the audio band, so the speaker does not reproduce it and the
  audio band is left with only the wanted tone.



When the module receives the address 0x04 from the bus, it loads two 16-bit
 frequency values. Each frequency drives its own 24-bit phase accumulator. To find how big
 the phase step ($\Delta\phi$) should be I would need to divide by the clock frequency, but
 a hardware divider is costly on an FPGA. So, using some online source
 (\url{https://electronics.stackexchange.com/questions/55493/fixed-point-division-in-verilog-for-spartan-6}),
 instead of dividing I multiply by a precomputed constant $K$ and then shift the bits to the
 right, which gives the same result: in my code this is $(f\cdot K + 2^{23})\gg24$.

At every single 40.5\,MHz clock tick, both 24-bit accumulators add their specific step
 ($\Delta\phi$) to their running total.
 However, the SINE\_LUT only has 256 entries, meaning it 
 only needs an 8-bit index to be read ($2^8 = 256$). 
 So the system uses only the top 8 bits of the 24-bit accumulator to pick the sample in the
 table. The other 16 bits are the part after the point: they do not choose a sample, they
 keep track of how far the phase has moved between one sample and the next. This is why the
 accumulator is 24 bit and not 8: the 8 top bits pick the sample, but the full 24 bits are
 what let me set the frequency in fine steps. The frequency step is $f_\mathrm{clk}/2^{24}$, a few Hz; with only
  ...
  same table at the exact same time. To send these two different
   frequencies to a single output pin, the code adds the two
...
\end{tikzpicture}}
\caption{Sinusoidal PWM (pwm\_generic\_master.vhd).}
\label{fig:spwm}
\end{figure}
...
The display is a small ST7789 TFT panel ($240\times320$ pixels, RGB565 colour) driven over
SPI. The FPGA block spi\_display.vhd is a transmit only SPI in mode 3 (SCK idles
high, the panel reads each bit on the rising edge). Only three wires carry data: SCK (the
clock), SDA (the bits) and CS (chip select). The clock uses a $\div6$ prescaler, so the SCK
is $40.5\,\mathrm{MHz}/6 = 6.75$\,MHz. In clock cycles, one SCK is 6 clk\_i and one
byte is $8\times6 = 48$ clk\_i $= 1.19\,\mu$s.

The panel also has three control pins, and the firmware sets them as simple register bits:
DC tells the panel whether a byte is a command or pixel data, RST resets the panel, and CS
picks it. The shift FSM has three states (Figure~\ref{fig:disp-fsm}): it waits for a byte,
then sends the 8 bits, and after the eighth bit SCK parks high, which is already the mode 3
idle, so there is no extra state between bytes.
...
function display\_tick() takes one job from the queue and sends only a small chunk
...
\end{tikzpicture}
\caption{Display SPI FSM (spi\_display.vhd).}
\label{fig:disp-fsm}
\end{figure}
...
\end{tikzpicture}
\caption{One display byte (spi\_display.vhd).}
\label{fig:disp-wave}
\end{figure}
...
The CPU manages NOR storage in firmware (norflash.c). The ESP32 sends one
character opcodes on the S7 UART (pins 72/20), and nor\_poll(), a non blocking
...
nor\_init() runs the boot chain, then the loop sits in the IDLE hub and
dispatches each opcode with a case (Figure~\ref{fig:flash-fsm}).

The chip uses the standard W25Q64 opcodes (Table~\ref{tab:nor-cmd}). Every access is one
SPI transaction: CS goes low, the master (spi\_master\_generic.vhd) clocks the
command byte MSB first, then a 24 bit address if the command needs one, then data, and CS
goes high. A read (0x03) sends the address and the chip drives the bytes back on
MISO (Figure~\ref{fig:nor-read}). A write is longer: a Write Enable (0x06) frame
first, then a Page Program (0x02) with the address and the data, then the master
polls the status register (0x05) until the WIP bit is 0
(Figure~\ref{fig:nor-write}). An erase works like the program, with 0x20 and no
data.

In clock cycles: SCK is clk\_i$/64 = 633$\,kHz, so one SCK is 64 clocks and one
...
\end{tikzpicture}
\caption{NOR flash read 0x03 (spi\_master\_generic.vhd).}
\label{fig:nor-read}
\end{figure}
...
\draw[line width=0.5pt] (0.6,\ysck-0.14)--(1.0,\ysck-0.14);
\clkspan{1.0}{2.35}{\ysck}{0.17}
\draw[line width=0.5pt] (2.35,\ysck-0.14)--(2.75,\ysck-0.14);
\clkspan{2.75}{8.15}{\ysck}{0.17}
\pcut{8.3}{\ysck-0.2}{\ysck+0.2}
\clkspan{8.65}{9.6}{\ysck}{0.17}
\draw[line width=0.5pt] (9.6,\ysck-0.14)--(10.0,\ysck-0.14);
\clkspan{10.0}{11.6}{\ysck}{0.17}
\draw[line width=0.5pt] (11.6,\ysck-0.14)--(11.9,\ysck-0.14);
\draw[line width=0.7pt] ({\xo-0.4},\ycs+0.2)--({\xo-0.1},\ycs+0.2)--({\xo-0.1},\ycs-0.2)
...
% command name right under its MOSI opcode byte
\node[font=\tiny, fill=white, inner sep=1pt] at ({\xo+0.5*\bw},\ymo-0.45){write enable};
\node[font=\tiny, fill=white, inner sep=1pt] at ({\xo+1.5*\bw+0.4},\ymo-0.45){page program};
\node[font=\tiny, fill=white, inner sep=1pt] at (10.4,\ymo-0.45){read status};
\end{tikzpicture}
\caption{NOR flash page program 0x02 (spi\_master\_generic.vhd).}
\label{fig:nor-write}
\end{figure}
...
\centering
\begin{tikzpicture}[every node/.style={font=\scriptsize},
  b/.style={draw, line width=0.7pt, fill=white, align=center, inner sep=3pt, minimum height=1.3cm, text width=2.3cm},
  a/.style={-{Stealth[length=5pt]}, line width=0.7pt}]
\node[b] (chip) at (0,0)    {W25Q64\\ 8\,MB (64 Mbit)};
\node[b] (blk)  at (3.7,0)  {one 64\,KB block\\ (128 blocks)};
\node[b] (sec)  at (7.4,0)  {one 4\,KB sector\\ (16 per block)};
\node[b] (pg)   at (11.1,0) {one 256\,B page\\ (16 per sector)};
\draw[a] (chip)-- node[above,font=\tiny]{zoom}(blk);
\draw[a] (blk)-- node[above,font=\tiny]{zoom}(sec);
\draw[a] (sec)-- node[above,font=\tiny]{zoom}(pg);
\node[font=\tiny] at (7.4,-1.1){smallest erase $=$ one 4\,KB sector};
\node[font=\tiny] at (11.1,-1.1){smallest write $=$ one 256\,B page};
\end{tikzpicture}
\caption{W25Q64 layout: blocks, sectors, pages.}
\label{fig:nor-chip}
\end{figure}

\textbf{Memory layout.} The W25Q64 is built from 64\,KB blocks, each split into 4\,KB
sectors, each split into 256\,B pages (Figure~\ref{fig:nor-chip}); the smallest erase is one
sector and the smallest write is one page. The firmware uses only the first 12\,KB
(Figure~\ref{fig:nor-map}): sector 0 (4\,KB) holds a 32 byte settings
blob that starts with a fixed byte 0xA5 (a marker the firmware checks to know the
settings are valid; if it is missing it uses defaults), then the device name, auto presence
interval, retry count, debug
flag, display theme colours, view flags, v2 marker 0x5A); sectors 1 to 2 hold the
...
\begin{tikzpicture}[every node/.style={font=\scriptsize}]
\def\W{4.2}
\draw[line width=0.7pt, fill=black!5] (0,0) rectangle (\W,1.0);
\draw[line width=0.7pt] (0,1.0) rectangle (\W,3.0);
\draw[line width=0.7pt] (0,3.0) rectangle (\W,4.0);
\node at (\W/2,0.5) {sector 0: settings};
\node at (\W/2,2.0) {sectors 1--2: log ring};
\node at (\W/2,3.5) {sectors 3 and up: free};
\node[anchor=east,font=\tiny] at (-0.1,0)   {\texttt{0x00\_0000}};
\node[anchor=east,font=\tiny] at (-0.1,1.0) {\texttt{0x00\_1000}};
\node[anchor=east,font=\tiny] at (-0.1,3.0) {\texttt{0x00\_3000}};
\node[anchor=west,font=\tiny,align=left] at (\W+0.2,0.5) {32 bytes: valid marker \texttt{0xA5}, name,\\ intervals, flags, theme, version \texttt{0x5A}};
\node[anchor=west,font=\tiny,align=left] at (\W+0.2,2.0) {512 slots $\times$ 16 B: seq, type,\\ time, value, 8 ASCII bytes};
\end{tikzpicture}
\caption{How the firmware organises the flash.}
\label{fig:nor-map}
\end{figure}

\textbf{Unlock before reading the id.} On this board the /HOLD and /WP
pins are left unconnected. A floating /HOLD pin picks up noise from the SCK line,
which makes the chip stop in the middle of a transfer, and then every read gives
0xFF, even the chip id. So I cannot just read the id first to check that the chip is
there. Instead, at boot the firmware first sends a WRSR command that sets the QE bit (this
frees the /HOLD and /WP pins) and clears the write protection, then a
Global Block Unlock, and only after that it reads the id. If the id is still wrong, the
firmware marks the chip as missing, so the later commands fail quickly instead of waiting a
long time. The timeouts are measured in real time, not in loop counts: 2\,ms per SPI byte,
700\,ms for the busy poll (an erase can take 400\,ms), and 200\,ms per byte coming from the
ESP32.
...
\caption{Storage FSM (norflash.c).}
\label{fig:flash-fsm}
\end{figure}

\textbf{Board notes.} During a sector erase the chip draws a short current burst, and on
the bare wiring this made the 3.3\,V rail drop enough to reset the chip in the middle of the
erase. To fix it I added two capacitors in parallel at the chip, one 1\,$\mu$F and one
10\,$\mu$F.
...
The FPGA has two identical UARTs from uart\_generic.vhd: the character UART (S6,
pins 17/18) that carries diagnostics and banner text, and the NOR command UART (S7, pins
72/20) that carries the one character flash opcodes (the full set is in
Figure~\ref{fig:flash-fsm}). They are two instances of the same
block with the same register map (Table~\ref{tab:addrmap}): 0x00 writes a byte to
send and reads the received byte (bit 8 is the valid flag), 0x04 and 0x08
enable and disable, 0x0C sets the baud divider and 0x10 the format, and
0x14 is the status (rx\_valid, tx\_busy). At 115\,200 baud the
...
transmit, a write to 0x00 sets tx\_busy and the machine sends a start
bit, 8 data bits LSB first, an optional parity bit and a stop bit
(Figure~\ref{fig:uart-wave}). On receive, two flip flops (rx\_q0,
rx\_q1) synchronise the line; a falling edge starts the machine, it samples the
first bit at half a bit time and the rest one bit time apart, and each finished byte goes
into a 16 deep FIFO (rxf) that the master reads from 0x00.
...
\end{tikzpicture}
\caption{UART frame, 8N1 (uart\_generic.vhd). One bit is baud\_div
...
\end{tikzpicture}}
\caption{UART transmit and receive state machines (uart\_generic.vhd).}
\label{fig:uart-fsm}
\end{figure}
...
The soft core runs a small firmware written in C (seven files plus a startup file), built
for rv32imc. The code sits in the 32\,KB ITCM and the data in the 16\,KB DTCM, both
local to the CPU and off the bus. The build turns the binary into a hex file that the CPU
loads into the ITCM at start up, so the firmware travels inside the FPGA bitstream and needs
no external memory.

The firmware is one main loop. Each pass of the loop calls a few short functions, one after
the other, always in this order (Figure~\ref{fig:firmware-loop}). First emit\_capture
takes the characters that arrived from the ESP32 and puts them in a queue. Then
emit\_player\_tick takes one character from the queue and plays its tone pair on the
speaker PWM (144\,ms of tone, then 400\,ms of silence); it also listens, so if it has heard
the other device in the last 3\,s it holds the queue, and if the other device starts talking
in the middle of a tone it stops the PWM at once. Next decode\_step checks
fill\_cnt, and when a new spectrum is ready it copies the snapshot, decodes it, and
sends the decoded characters to the ESP32. After that nor\_poll runs the flash
storage protocol (Section~\ref{sec:flash}), and finally display\_tick sends one small
chunk to the display (Section~\ref{sec:display}).

A health\_tick runs every 15\,s to check that the slaves are alive. It reads one safe
register from each slave and looks at the value it gets back; if a slave gives a wrong or
stuck value it is marked as not healthy. The result is packed into a short status word
(\$HLT) that both the display and the web dashboard show, so I can see at a glance
which blocks are working. The same idea also helped at boot: just before the firmware talks
to a slave for the first time, it prints a small marker on the diagnostics UART. If that
slave locks up the CPU, the last marker printed is the name of the slave that caused it, so I
know straight away where the problem is. This is how I found the GPIO ack bug in a few
minutes.

Diagnostics use the character UART. They are simple lines that start with \$ and end
with a newline; the ESP32 sends these to the USB console instead of to the protocol parser. A
debug flag in the NOR settings, which can be switched from the dashboard, turns them on and
off.
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[every node/.style={font=\scriptsize},
  b/.style={draw, line width=0.7pt, fill=white, align=center, rounded corners=1pt, inner sep=3pt, minimum height=1.15cm, text width=2.3cm},
  a/.style={-{Stealth[length=5pt]}, line width=0.7pt}]
\node[b] (cap) at (0,0)    {\texttt{emit\_capture}\\[1pt] \tiny queue chars from ESP32};
\node[b] (ply) at (3.1,0)  {\texttt{emit\_player\_tick}\\[1pt] \tiny play tone, listen};
\node[b] (dec) at (6.2,0)  {\texttt{decode\_step}\\[1pt] \tiny decode spectrum};
\node[b] (nor) at (9.3,0)  {\texttt{nor\_poll}\\[1pt] \tiny flash protocol};
\node[b] (dsp) at (12.4,0) {\texttt{display\_tick}\\[1pt] \tiny one display chunk};
\draw[a] (cap)--(ply); \draw[a] (ply)--(dec); \draw[a] (dec)--(nor); \draw[a] (nor)--(dsp);
\draw[a] (dsp) to[out=-90,in=-90,looseness=0.35] node[below,font=\tiny,pos=0.5]{next pass of the loop} (cap);
\node[font=\tiny] at (6.2,1.05){one pass of the main loop (no OS, no interrupts); every call returns at once};
\end{tikzpicture}}
\caption{The firmware main loop.}
...
The ESP32 handles the protocol logic, the queue of incoming and outgoing messages, and the
link with the web interface. It does not touch the FPGA logic directly: the two chips talk
only over two UART links (Figure~\ref{fig:esp-link}), and they share a common ground.

The first link is the character UART (FPGA pins 17 and 18): the FPGA sends the decoded
protocol characters and the diagnostics, and the ESP32 sends back the characters to
transmit. The second link is the NOR command UART (FPGA pins 72 and 20): the ESP32 sends the
one character flash opcodes and the FPGA firmware runs them. Both links are 8N1 at
115\,200 baud.

\begin{figure}[H]
\centering
\begin{tikzpicture}[every node/.style={font=\scriptsize},
  b/.style={draw, line width=0.8pt, fill=white, align=center, minimum width=2.6cm, minimum height=3.4cm},
  a/.style={-{Stealth[length=5pt]}, line width=0.7pt}]
\node[b] (fpga) at (0,0) {\textbf{FPGA}\\ Tang Nano};
\node[b] (esp)  at (8,0) {\textbf{ESP32}};
\draw[a] (1.3,1.3) -- node[above,font=\tiny]{pin 17 char TX $\to$ RX} (6.7,1.3);
\draw[a] (6.7,0.7) -- node[above,font=\tiny]{pin 18 char RX $\gets$ TX} (1.3,0.7);
\draw[a] (1.3,-0.2) -- node[above,font=\tiny]{pin 72 flash TX $\to$ RX} (6.7,-0.2);
\draw[a] (6.7,-0.8) -- node[above,font=\tiny]{pin 20 flash RX $\gets$ TX} (1.3,-0.8);
\draw[line width=1.0pt] (1.3,-1.5) -- node[above,font=\tiny]{common GND} (6.7,-1.5);
\node[font=\tiny] at (4.0,1.75){character UART (S6)};
\node[font=\tiny] at (4.0,0.25){NOR command UART (S7)};
\end{tikzpicture}
\caption{ESP32 to FPGA: two UART links and a shared ground.}
\label{fig:esp-link}
\end{figure}

The character UART carries the protocol symbols (both ways) and the diagnostics, while the
NOR command UART carries the one character flash opcodes. Table~\ref{tab:esp-opcodes} lists
the bytes that go over each link.

\begin{table}[H]
\centering
\caption{Bytes exchanged on the two links.}
\label{tab:esp-opcodes}
\begin{tabular}{lll}
\hline
Link & Byte & Meaning \\
\hline
character UART & protocol symbols & the tones, decoded or to send (Section~\ref{sec:protocol}) \\
...
On the ESP32 side the software is a few small modules: a UART driver that writes the chip
registers directly (115\,200 baud from the 80\,MHz clock), a flash link with retry and read
back, the codebook, the protocol state machine, and the WebSocket client for the dashboard.
The Wi-Fi transmit power is kept at 11\,dBm, because at full power the current bursts once
tripped the brownout detector on a weak supply.
...
change of tone, easy to see, rather than on silence, which is hard to
measure on a noisy channel. Six symbols exist: lowercase a,b,c (bit 0,
bit 1, EOF) ride the 2014\,Hz master carrier (FFT bin 22), uppercase
A,B,C ride the 2472\,Hz slave carrier (bin 27); EOF, bit 0 and bit 1 use
...
On the air, each transmission adds three things (Figure~\ref{fig:tx-timeline}). First, a
warm up symbol: the EOF tone is held for 300\,ms instead of 144\,ms, which gives the
receiving microphone time to settle; it carries no data. Second, the EOF that frames the
message is sent four times in a row, because losing an EOF breaks the whole message, while
losing a data bit only loses one bit. Third, each symbol is 144\,ms of tone followed by
400\,ms of silence; a shorter gap would let the echo of one tone run into the next and merge
two equal bits into one. The receiver does the opposite: it only accepts a decoded symbol if
it is different from the last one it accepted, which turns the repeated tones and the four
EOFs back into single symbols. If a message is left half received with no new symbol for
2\,s, it is dropped.
...
Only one device talks at a time. The master checks this in two ways. First, the soft core
listens: if it hears the other device, it holds the tone player and stops a tone that is
playing. Second, before sending each byte the ESP32 waits for 1\,s of silence, and it gives
up after 15\,s so a noisy line cannot block sending forever. The channel loses tones often,
so both sides retry: a presence request with no answer for 8\,s is sent again up to a set
number of times (default 6, saved in flash), and the registration message is sent again on a
random 30 to 40\,s timer until it is confirmed. Receiving the same message twice does no
harm, so repeats are safe.
