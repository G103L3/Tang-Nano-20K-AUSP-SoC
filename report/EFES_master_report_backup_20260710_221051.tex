...
% data_ready: direct wire from the ADC SPI (S1) to the capture FSM, not through the bus
\draw[densely dashed, -{Stealth[length=5pt]}, line width=0.7pt] (R2.north) |- (-5.15,6.1);
\node[font=\scriptsize, fill=white, inner sep=1.5pt] at (0.8,6.1) {\texttt{data\_ready} $\to$ \texttt{dma\_proc} (direct wire)};
...
The board runs on three rails (Figure~\ref{fig:power}). A USB-C PD trigger gives 12\,V. A
DFR1202 buck steps the 12\,V down to 5\,V for the board \texttt{VIN} and for the analog
front end, and an on board regulator makes the 3.3\,V for the digital parts. The 12\,V also
feeds the speaker stage directly.
...
\node[b] (spk) at (3.3,-2.0) {speaker stage\\ (low side)};
\node[b] (afe) at (6.9,-2.0) {analog\\ front end};
...
\draw[a] (usb.south) |- node[pos=0.95,above,font=\tiny]{12\,V}(spk.west);
\draw[a] (buck.south) |- node[pos=0.95,above,font=\tiny]{5\,V}(afe.west);
\end{tikzpicture}
\caption{Power tree.}
\label{fig:power}
\end{figure}
...
The display is the fourth SPI and the simplest (\texttt{spi\_display.vhd}): transmit
only, mode 3 (SCK idles high, the panel samples on the rising edge), $\div6$
prescaler $=6.75$\,MHz, it can afford to be the fastest SPI because the panel
sits centimetres away on its own wires. In clock cycles, one SCK is 6 \texttt{clk\_i} and
one byte is $8\times6 = 48$ \texttt{clk\_i} $= 1.19\,\mu$s. DC, RST and CS are register
bits; the FSM is three states (Figure~\ref{fig:disp-fsm}), and after the eighth bit SCK
parks high, which \emph{is} the mode 3 idle, so byte boundaries need no extra
states. On top of it the firmware runs a small dashboard: after the ST7789 init
sequence (SWRESET, SLPOUT, COLMOD RGB565, MADCTL landscape, INVON, NORON, DISPON,
each with its datasheet delay), drawing requests become jobs in a 48 entry queue and
each \texttt{display\_tick()} pushes at most one chunk of 128 to 384 SPI bytes
($150$, $450\,\mu$s), invisible next to the 10.9\,ms frame, a full screen would
otherwise block the loop for $180$\,ms, stalling the tone player and
overflowing the command FIFO. Four fixed zones show slave health, the live spectrum
