#pragma once
#include <stdint.h>

// ============================================================
// Mappa periferiche di sistema
// ============================================================
// CPU memory map:
//   0x01000000  DTCM  - data RAM (stack, variabili)
//   0x02000000  ITCM  - istruzioni (firmware caricato via JTAG)
// WB esterno (attivo per addr[31:28]>0 e bit31=0):
//   0x10000000  S0    - SDRAM
//   0x20000000  S6    - UART_GENERIC esterno
//   0x30000000  S5    - DMA control
//   0x40000000  S1    - SPI Master
//   0x50000000  S2    - PWM 10-bit
//   0x60000000  S3    - PWM 4-bit
//   0x70000000  S4    - GPIO 1-bit
// ============================================================

// --- UART_GENERIC esterno (0x20000000) ---
// +0x00 RD: bit[8]=rx_valid, bit[7:0]=dato RX (lettura azzera rx_valid)
// +0x00 WR: trasmetti byte (ignorato se tx_busy=1 o enabled=0)
// +0x04 WR: abilita UART (start)   [word 1]
// +0x08 WR: disabilita UART (stop)  [word 2]
// +0x0C WR: dat[15:0] = baud_div = clk_hz / baud_rate  (default 234 = 27MHz/115200) [word 3]
// +0x10 WR: dat[1:0]=parity(00=none,01=even,10=odd), dat[2]=stop_bits(0=1,1=2),
//           dat[6:3]=data_bits-5 (0=5bit … 3=8bit … 4=9bit) [word 4]
// +0x14 RD: dat[1]=rx_valid, dat[0]=tx_busy [word 5]
// NOTE: all register offsets are multiples of 4 (word-aligned) so the compiler
//       generates sw/lw and data lands in dat_i[7:0] as the VHDL expects.
#define UARTEXT_BASE        0x20000000u
#define UARTEXT_DATA        (*(volatile uint32_t*)(UARTEXT_BASE + 0x00))
#define UARTEXT_START       (*(volatile uint32_t*)(UARTEXT_BASE + 0x04))
#define UARTEXT_STOP        (*(volatile uint32_t*)(UARTEXT_BASE + 0x08))
#define UARTEXT_DIV         (*(volatile uint32_t*)(UARTEXT_BASE + 0x0C))
#define UARTEXT_CFG         (*(volatile uint32_t*)(UARTEXT_BASE + 0x10))
#define UARTEXT_STATUS      (*(volatile uint32_t*)(UARTEXT_BASE + 0x14))

#define UARTEXT_CFG_PARITY_NONE  0x00u
#define UARTEXT_CFG_PARITY_EVEN  0x01u
#define UARTEXT_CFG_PARITY_ODD   0x02u
#define UARTEXT_CFG_STOP2        0x04u
#define UARTEXT_CFG_BITS(n)      (((uint32_t)((n)-5) & 0xFu) << 3)

static inline void uartext_init(uint32_t clk_hz, uint32_t baud, uint32_t cfg) {
    UARTEXT_STOP  = 1;
    UARTEXT_DIV   = clk_hz / baud;
    UARTEXT_CFG   = cfg;
    UARTEXT_START = 1;
}
static inline void uartext_putchar(char c) {
    while (UARTEXT_STATUS & 0x1u) {}
    UARTEXT_DATA = (uint8_t)c;
}
static inline int uartext_getchar_nb(void) {
    uint32_t d = UARTEXT_DATA;
    return (d & (1u << 8)) ? (int)(d & 0xFF) : -1;
}
static inline void uartext_puts(const char *s) {
    while (*s) uartext_putchar(*s++);
}

// --- SPI Master (0x40000000) ---
// 0x00 RD: 32-bit dato MISO (ack solo se data_ready=1)
// 0x04 WR: avvia lettura SPI (start)   [word 1]
// 0x08 WR: ferma lettura SPI (stop)    [word 2]
// 0x0C WR: azzera data_ready flag      [word 3]
#define SPI_BASE        0x40000000u
#define SPI_DATA        (*(volatile uint32_t*)(SPI_BASE + 0x00))
#define SPI_START       (*(volatile uint32_t*)(SPI_BASE + 0x04))
#define SPI_STOP        (*(volatile uint32_t*)(SPI_BASE + 0x08))
#define SPI_CLR_READY   (*(volatile uint32_t*)(SPI_BASE + 0x0C))

static inline uint32_t spi_read32(void) {
    SPI_START = 1;
    // aspetta data_ready (il WB rimane in stall finché non c'è dato)
    uint32_t d = SPI_DATA;
    SPI_STOP = 1;
    SPI_CLR_READY = 1;
    return d;
}

// --- PWM 15-bit (0x50000000) ---
// 0x04 WR: dat[29:15]=duty, dat[14:0]=period → avvia  [word 1]
// 0x08 WR: ferma                                        [word 2]
// period = CLK_HZ / freq_hz  (e.g. 27e6/1000 = 27000 → fits in 15 bits up to ~824 Hz min)
// For audio 1kHz–9kHz: period = 27000–3000, well within 15 bits (max 32767)
#define PWM10_BASE      0x50000000u
#define PWM10_START_REG (*(volatile uint32_t*)(PWM10_BASE + 0x04))
#define PWM10_STOP_REG  (*(volatile uint32_t*)(PWM10_BASE + 0x08))

static inline void pwm10_start(uint32_t period_15b, uint32_t duty_15b) {
    PWM10_STOP_REG  = 1;
    PWM10_START_REG = ((duty_15b & 0x7FFFu) << 15) | (period_15b & 0x7FFFu);
}
static inline void pwm10_stop(void) { PWM10_STOP_REG = 1; }

// --- PWM 4-bit (0x60000000) ---
// 0x04 WR: dat[7:4]=duty, dat[3:0]=period → avvia  [word 1]
// 0x08 WR: ferma                                     [word 2]
#define PWM4_BASE       0x60000000u
#define PWM4_START_REG  (*(volatile uint32_t*)(PWM4_BASE + 0x04))
#define PWM4_STOP_REG   (*(volatile uint32_t*)(PWM4_BASE + 0x08))

static inline void pwm4_start(uint8_t period_4b, uint8_t duty_4b) {
    PWM4_STOP_REG  = 1;
    PWM4_START_REG = ((uint32_t)(duty_4b & 0xF) << 4) | (period_4b & 0xF);
}
static inline void pwm4_stop(void) { PWM4_STOP_REG = 1; }

// --- GPIO 1-bit (0x70000000) ---
// WR: dat[0] = gpio_1_o
// RD: dat[0] = gpio_in (attualmente fissato a 0 nel top)
#define GPIO_BASE       0x70000000u
#define GPIO_REG        (*(volatile uint32_t*)(GPIO_BASE))

static inline void gpio_set(uint32_t val) { GPIO_REG = val & 1u; }

// --- DMA SPI→SDRAM + FFT (0x30000000) ---
// +0x04 WR: start   — avvia acquisizione continua MCP3201 (12-bit, ~46.875 kHz) [word 1]
// +0x08 WR: stop    — ferma acquisizione                                          [word 2]
// +0x0C WR: base    — imposta indirizzo base SDRAM (bit[20:0] = word address 21-bit) [word 3]
// Ogni 512 campioni scatta FFT a 512 punti sul blocco appena scritto.
// Risultati xk_re scritti in SDRAM da word address 0x1300 (byte 0x10004C00).
// IRQ bit 20 alzato al termine della FFT; il main legge da FFT_RESULT_BASE.
// Ping-pong: blocco A = base..base+511, blocco B = base+512..base+1023, poi wrap.
#define DMA_BASE        0x30000000u
#define DMA_START       (*(volatile uint32_t*)(DMA_BASE + 0x04))
#define DMA_STOP        (*(volatile uint32_t*)(DMA_BASE + 0x08))
#define DMA_SETBASE     (*(volatile uint32_t*)(DMA_BASE + 0x0C))

// --- SDRAM (0x10000000) 
#define SDRAM_BASE      ((volatile uint32_t*)0x10000000u)
