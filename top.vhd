...
begin
    ...
    port map (
        ser_tx => ser_tx, ser_rx => ser_rx, slv_ext_stb_o => cpu_stb, slv_ext_we_o => cpu_we,
        slv_ext_cyc_o => cpu_cyc, slv_ext_ack_i => cpu_ack, slv_ext_adr_o => cpu_adr,
        slv_ext_wdata_o => cpu_wdata, slv_ext_rdata_i => cpu_rdata, slv_ext_sel_o => cpu_sel,
        irq_in => irq_dummy, jtag_tdi => jtag_tdi, jtag_tdo => jtag_tdo, jtag_tck => jtag_tck,
        jtag_tms => jtag_tms, clk_in => clk_i, resetn_in => rst_i
    );
    ...
    port map (
        ...
        s4_adr_o => s4_adr, s4_dat_o => s4_wdata, s4_dat_i => s4_rdata, s4_we_o => s4_we, s4_sel_o => s4_sel, s4_stb_o => s4_stb, s4_cyc_o => s4_cyc, s4_ack_i => s4_ack,
    ...
    u_spi: spi_master
    port map (
        clk_i => clk_i, rst_i => rst_i, cyc_i => s1_cyc, stb_i => s1_stb, we_i => s1_we,
    ...
    port map (
        ...
        adr_i => s2_adr(7 downto 0), dat_i => s2_wdata, dat_o => s2_rdata, ack_o => s2_ack, pwm_o => pwm_10_o
    );
    ...
    port map (
        ...
        adr_i => s3_adr(7 downto 0), dat_i => s3_wdata, dat_o => s3_rdata, ack_o => s3_ack, pwm_o => pwm_4_o
    );
